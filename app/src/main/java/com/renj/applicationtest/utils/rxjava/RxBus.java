...
public class RxBus {
     ...
     * @return
     */
    public <T> Flowable<T> toFlowable(@NonNull Class<T> tClass) {
     ...
     * @return
     */
    public <T> Flowable<T> toFlowableSticky(@NonNull final Class<T> tClass) {
        synchronized (mStickyEventMap) {
            Flowable<T> flowable = mBus.ofType(tClass);
            if (tClass != null) {
                final Object obj = mStickyEventMap.get(tClass);
                return flowable.mergeWith(new Flowable<T>() {
                    @Override
                    protected void subscribeActual(Subscriber<? super T> s) {
                        s.onNext(tClass.cast(obj));
                    }
                });
            } else {
                return flowable;
    ...
    public <T> Disposable toDefaultFlowable(Class<T> tClass, Consumer<T> consumer) {
        return mBus.ofType(tClass)
                .compose(RxUtils.newInstance().<T>threadTransformer())
                .subscribe(consumer);
    }
}
